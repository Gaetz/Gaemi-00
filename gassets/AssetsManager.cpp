...
namespace gassets {
    ...
    void AssetsManager::Initialize(const str& gameAssetsPath) {
        ...
        // Default assets
        LoadSystemTexture(DEFAULT_TEXTURE_ID, "invalid-texture.png");
        LoadSystemModel(DEFAULT_MODEL_ID);
    }
    ...
    void AssetsManager::LoadTextureCubemap(const str& name, const str& filename, CubemapTextureLayout layout,
                                           i32 sceneId) {
        str path = gfile::FileTypePath(gfile::FileType::Texture) + filename;
    ...
    Texture& AssetsManager::GetTexture(const str& name) {
        if (!textures.contains(name)) {
            LOG(LogLevel::Error) << "Texture [" << name <<
 "] does not exist in AssetsManager. Returning default texture.";
    ...
    Model3D& AssetsManager::GetModel(const str& name) {
        if (!models.contains(name)) {
            ...
            return models.at(DEFAULT_MODEL_ID);
        }
